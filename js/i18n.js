// i18n.js - Internationalization engine with embedded translations
(function (global) {
    'use strict';
    ...
    function getAvailableLangs() { return Object.keys(TRANSLATIONS); }

    // Locale-aware number formatting (Spanish: 1.000,5 / English: 1,000.5)
    function fmt(n) {
        const num = Number(n);
    ...
    function fmtDec(n, decimals) {
        ...
        const tSep  = currentLang === 'es' ? '.' : ',';
        const dSep  = currentLang === 'es' ? ',' : '.';
        return int.replace(/\B(?=(\d{3})+(?!\d))/g, tSep) + (decimals > 0 ? dSep + dec : '');
    }

    global.i18n = { t, getLang, setLang, getAvailableLangs, fmt, fmtDec };
